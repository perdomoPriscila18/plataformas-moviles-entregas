...
function encontrarPalabraMasCorta(listaDePalabras) {
    ...
    return listaDePalabras.reduce((masCorta, actual) => {
        return actual.length < masCorta.length ? actual : masCorta;
    });
}
...
function contarAprobados(listaDeNotas) {
    const NOTA_APROBACION = 4; // Ajusta este número según tu criterio de aprobación
    return listaDeNotas.filter(nota => nota >= NOTA_APROBACION).length;
}
...
function filtrarSoloTruthy(listaDeValores) {
    return listaDeValores.filter(Boolean); // 'Boolean' actúa como función de filtrado directo
}
...
function enumerarLista(listaDePalabras) {
    ...
    if (listaDePalabras.length === 1) {
        let unica = listaDePalabras[0];
        return unica.charAt(0).toUpperCase() + unica.slice(1) + ".";
    }

    const ultimos = [...listaDePalabras];
    const ultimoElemento = ultimos.pop();
    
    let oracion = ultimos.join(", ") + " y " + ultimoElemento;
    
    // Aplicamos la primera letra en mayúscula y agregamos punto al final
